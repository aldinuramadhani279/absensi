...
interface Attendance {
    id: number;
    user: { name: string; profession: { name: string } };
    shift: { name: string };
    date: string;
    clock_in: string;
...
export default function ReportsIndex({ professions, attendances, filters }: Props) {
                    ...
                    <CardContent>
                        <Table>
                            ...
                            ${att.status === 'tepat waktu' ? 'bg-green-100 text-green-700' :
                                                    att.status === 'terlambat' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}
                        `}>
                                                {att.status}
                                            </span>
                                        </TableCell>
                                    ...
                                    <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">Tidak ada data untuk ditampilkan. Silakan gunakan filter di atas.</TableCell></TableRow>
                                )}
                            </TableBody>
                        </Table>
                    </CardContent>
